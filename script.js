...
async function hydrateAbout(){
  try{
    ...
    if(grid){
      grid.innerHTML = '';
      (press.items||[]).forEach(p=>{
        const a = document.createElement('a');
        a.href = p.link || '#'; a.target = '_blank'; a.rel = 'noopener';
        a.className = 'card-hover flex items-center justify-center bg-white rounded-2xl p-6';
        const img = document.createElement('img');
        img.src = p.image; img.alt = p.alt||'press';
        img.style.maxHeight = '48px'; img.style.width='auto';
        a.appendChild(img);
        grid.appendChild(a);
      });
    }
  }catch(e){ console.log('hydrateAbout failed', e); }
}
async function hydrateGallery(){
  try{
    ...
    if(grid){
      const list = gal.images||[];
      if(list.length){
        grid.innerHTML='';
        list.forEach(item=>{
          const url = (item.image||item).toString();
          const img = document.createElement('img');
          img.src = url; img.alt='portfolio'; img.className='card-hover';
          grid.appendChild(img);
        });
      }
    }
  }catch(e){ console.log('hydrateGallery failed', e); }
}
async function hydrateServices(){
  try{
    ...
    if(grid){
      ...
      (s.items||[]).forEach(svc=>{
        ...
        card.innerHTML = `<div class="text-sm text-fuchsia-600 font-semibold">Signature</div>
  ...
  }catch(e){ console.log('hydrateServices failed', e); }
}
async function hydrateFAQ(){
  try{
    ...
    if(wrap){
      ...
      list.forEach(qa=>{
        const card = document.createElement('div');
        card.className = 'card';
        card.innerHTML = `<button class="faq-q flex items-center justify-between w-full text-left font-semibold">
          <span>${qa.q}</span>
  ...
  }catch(e){ console.log('hydrateFAQ failed', e); }
}
async function hydrateStudents(){
  ...
  }catch(e){ console.log('hydrateStudents failed', e); }
}
async function hydrateTutorials(){
  try{
    ...
    function render(){
      if(grid){
        ...
        list.forEach(it=>{
          ...
          card.innerHTML = `
            <img src="${it.thumb||('https://img.youtube.com/vi/'+vid+'/hqdefault.jpg')}" class="tutorial-thumb" alt="${it.title}">
            <div class="mt-3">
    ...
    render();
  }catch(e){ console.log('hydrateTutorials failed', e); }
}
async function hydrateProducts(){
  try{
    ...
    function render(){
      grid.innerHTML='';
      (data.items||[]).filter(p => current==='All' or p.category===current).forEach(p=>{
        const card = document.createElement('div');
        card.className = 'card card-hover';
        card.innerHTML = `
    ...
    if(chipWrap){
      ...
      all.addEventListener('click',()=>{ current='All'; [...chipWrap.children].forEach(c=>c.classList.remove('active')); all.classList.add('active'); render(); });
      cats.forEach(cat=>{
        const b = document.createElement('button');
        b.className='chip'; b.textContent=cat;
        b.addEventListener('click',()=>{ current=cat; [...chipWrap.children].forEach(c=>c.classList.remove('active')); b.classList.add('active'); render(); });
        chipWrap.appendChild(b);
      });
    }
    render();
  }catch(e){ console.log('hydrateProducts failed', e); }
}
async function hydrateTestimonials(){
  try{
    ...
    if(grid){
      ...
      (data.items||[]).forEach(t=>{
        ...
        const starsHTML = Array.from({length:5}).map((_,i)=> i<stars ? '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 .6l3.1 6.3 6.9 1-5 4.9 1.2 6.9L12 16.9 5.8 19.7 7 12.8 2 7.9l6.9-1z"/></svg>' : '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M12 2l3.1 6.3 6.9 1-5 4.9 1.2 6.9L12 16.9 5.8 19.7 7 12.8 2 7.9l6.9-1z"/></svg>').join('');
        card.innerHTML = `
  ...
  }catch(e){ console.log('hydrateTestimonials failed', e); }
}
document.addEventListener('DOMContentLoaded', ()=>{
  hydrateCommon();
  if(document.body.dataset.page==='about') hydrateAbout();
  if(document.body.dataset.page==='gallery') hydrateGallery();
  if(document.body.dataset.page==='faq') hydrateFAQ();
  if(document.body.dataset.page==='services') hydrateServices();
  if(document.body.dataset.page==='students') hydrateStudents();
  if(document.body.dataset.page==='tutorials') hydrateTutorials();
  if(document.body.dataset.page==='products') hydrateProducts();
  if(document.body.dataset.page==='testimonials') hydrateTestimonials();
